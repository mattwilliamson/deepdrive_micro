...
{
private:
  // TODO: Remove _debugSerial
    char *_debugSerial;     //The stream to send debug messages to if enabled
    bool _printDebug = false; //Flag to print the serial commands we are sending to the Serial port for debug

    uint8_t MAX_MAGNETOMETER_STARTS = 10; // This replaces maxTries
...
#endif
#else
  void enableDebugging(char *debugPort = NULL); //Given a port to print to, enable debug messages.
#endif
...
public:
  ...
  uint8_t _addr;
  uint8_t _ad0; // pin?
  bool _ad0val; // i2c address offset
  ICM_20948_Serif_t _serif;

  ICM_20948_I2C(); // Constructor

  virtual ICM_20948_Status_e begin(i2c_inst_t *_i2c = i2c_default, bool ad0val = true, uint8_t ad0pin = ICM_20948_ARD_UNUSED_PIN);
};

// Arduino shim for now
#define F(x) x
