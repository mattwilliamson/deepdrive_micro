...
void IMU::quaternianToEuler(madgwick_ahrs_t *data, float euler[]) {
  ...
  euler[0] = -1.0f * asinf(2.0f * (data->q[1]) * (data->q[3]) +
                           2.0f * (data->q[0]) * (data->q[2]));
  euler[1] = atan2f(
      2.0f * (data->q[2]) * (data->q[3]) - 2.0f * (data->q[0]) * (data->q[1]),
      2.0f * (data->q[0]) * (data->q[0]) + 2.0f * (data->q[3]) * (data->q[3]) -
          1.0f);
  euler[2] = atan2f(
      2.0f * (data->q[1]) * (data->q[2]) - 2.0f * (data->q[0]) * (data->q[3]),
      2.0f * (data->q[0]) * (data->q[0]) + 2.0f * (data->q[1]) * (data->q[1]) -
          1.0f);
  return;
}
...
Vector3 IMU::getAccel() { return {accel_g_[0], accel_g_[1], accel_g_[2]}; }

Vector3 IMU::getGyro() {
  return {gyro_dps_[0] / RAD_TO_DEG, gyro_dps_[1] / RAD_TO_DEG,
          gyro_dps_[2] / RAD_TO_DEG};
}

Vector3 IMU::getMag() { return {mag_ut_[0], mag_ut_[1], mag_ut_[2]}; }
