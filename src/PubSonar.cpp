...
void PubSonar::calculate() {
  ...
  // Timer is due, start the measurement
  if (!started_ && _pub_sonar_triggered[pin_trigger_]) {
    // mutex_exit(&lock_);
    started_ = true;
    sonar_->startAsync(100000);
    return;
  }
  ...
  // Read the sonar distance and convert to meters
  double s = sonar_->getDist_cm() / 100.0;
