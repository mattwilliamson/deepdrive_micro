...
LEDRing::LEDRing(uint8_t pin, PIO pio, uint8_t ledCount) : pin(pin), pio(pio), ledCount(ledCount) {
...
inline uint8_t LEDRing::gamma_u8(uint8_t x)
{
    return led_ring_gamma_table[x];
}

inline uint8_t LEDRing::sine(uint8_t x)
{
